...
description: 这个 skill 用于辅助 Agent 工程师和产品负责人进行生产级 Agent 系统规划、编排设计、工具/MCP 设计、代码 Review、安全评审和认知辅导。当用户需要把 Agent 想法拆成可落地架构、列举 Agent 脚手架与组件及何时添加、审查现有 Agent 代码、识别 Anti-Pattern、设计 memory/context/tool/security/skill 体系，或希望理解 Hermes Agent、Odysseus 等现代 Agent 工程实践时，应使用此 skill。
---
...
当用户描述一个 Agent 想法、业务流程或产品功能时，先读 `references/agent-scaffolding-components.md` 做 v1 裁剪，再读 D1、D2、D5、D8、D9；如果涉及 Hermes/Odysseus/本地工作台/skill 生态，再读 D6、D7。

输出必须覆盖：
1. 目标和边界：Agent 要解决什么，不解决什么。
2. 架构选型：单 Agent、Coordinator + Subagents、固定链路、动态分解、human-in-the-loop。
