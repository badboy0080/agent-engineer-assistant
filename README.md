...
| **认知辅导** | 用产品经理能理解的话解释 Agent 编排、MCP、memory、tool policy、脚手架与组件等概念 | 团队需要统一 Agent 构建认知 |
...
| 索引 | 脚手架与组件全景 | 脚手架/组件清单、何时添加、PoC→生产阶段裁剪 |
...
> "Agent 构建里有哪些脚手架和组件？什么时候该加？"

> "请用产品经理能听懂的话解释，为什么 Agent 工具权限不能只靠 prompt 约束？"

Skill 会先读 `references/agent-scaffolding-components.md` 做总览，再按场景深入 Domain；先讲业务风险，再映射到工程机制。
...
├── README.md                         # 本文件
└── references/                       # 领域规范与索引文档
    ├── agent-scaffolding-components.md # 脚手架与组件全景索引（何时添加、阶段裁剪）
    ├── acceptance-test-checklist.md  # Skill 验收与回归测试清单
